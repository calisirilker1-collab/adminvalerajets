(() => {
  ...
  async function loadDealRelations(){
    ...
    const [oq,cq,tl]=await Promise.all([
    ...
    for(const result of [oq,cq,tl]) if(result.error) console.error(result.error);
    dealOperatorQuotes=oq.data||[]; dealClientQuotes=cq.data||[]; dealTimeline=tl.data||[];
    $('operatorCount').textContent=dealOperatorQuotes.length; $('clientQuoteCount').textContent=dealClientQuotes.length; $('timelineCount').textContent=dealTimeline.length;
  }

  function renderAllDealPanels(){ renderOverview(); renderOperators(); renderClientQuotes(); renderFinance(); renderFollowup(); renderTimeline(); }
  ...
  $('refreshButton').addEventListener('click',async()=>{await loadRequests();notify('Talepler yenilendi.');});
  ...
  document.querySelectorAll('.nav-item').forEach(btn=>btn.addEventListener('click',()=>{activeFilter=btn.dataset.filter;statusSelect.value='all';document.querySelectorAll('.nav-item').forEach(n=>n.classList.toggle('active',n===btn));$('panelTitle').textContent=btn.textContent.replace(/\d+/g,'').trim();renderRows();}));
